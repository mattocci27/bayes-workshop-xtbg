...
- Because we usually don't know the true parameters ($\mu$ and $\sigma$), we need to find the parameters that maximize the likelihood function (**MLE: Maximum Likelihood Estimation**).
  ...
  - e.g., for a liner model $y_i = ax_i + b$, we usually assume that $y_i \sim \mathcal{N}(\mu_i = ax_i + b, \sigma^2)$, and we want to find the parameters $a$, $b$, and $\sigma$ that maximize the likelihood function: $P(y_i \mid ax_i + b, \sigma)$
:::
...
- Bayesian methods are inherently designed for hierarchical (multilevel) modeling thourgh priors.
...
- $H(\theta, r) = U(\theta) + r^2/2$ (Hamitonian: potential energy + kinetic energy = constant)

- Contour lines of $H(\theta, r)$ in the phase space ($\theta$ vs. $r$)

- Instead of sampling from $p(\theta \mid \text{data})$, HMC samples from a joint distribution over both.
...
$$
p(\theta) \propto \int exp(-H(\theta, r))dr \propto exp(-U(\theta))
$$
...
Why parameter in the same trajectory is better than random parameters?

theta in the same trajectory may have the the same $p(\theta \mid data)$ because of this?
