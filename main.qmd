...
anim_save("images/mcmc_trace_simple.gif", animation = p1_ani)

p2_ani <- animate(p2, nframes = n_iter, fps = 15, width = 600, height = 400)
anim_save("images/mcmc_post_simple.gif", animation = p2_ani)
```

![](images/mcmc_trace_simple.gif)

![](images/mcmc_post_simple.gif)

:::

::: {.column width="50%"}

- Start with some values of *p* (say 0.2 and 0.8)
- Caclulate the posterior.
- Repeat the followings N times:
  - Propose a new value of *p* (say, *p* + 0.01 or *p* - 0.01)
  - Calculate the posterior for the new *p*.
  - If the new posterior is better (higher), accept the new *p*; otherwise, keep the current *p*.
:::
:::
