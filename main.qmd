...
- Interefaces to Stan
- Other tools for Bayesian modeling
...
set.seed(123)
n_iter <- 200
chain1 <- mcmc_simple(n_iter, init_p = 0.2)
chain2 <- mcmc_simple(n_iter, init_p = 0.8)
...
# animated trace
p1 <- ggplot(df, aes(x = iteration, y = p, color = chain)) +
  geom_line(lwd = 2) +
  geom_point(size = 4) +
  labs(
...
# animated post
p2 <- ggplot(df, aes(x = p, y = post, color = chain)) +
  geom_line(lwd = 2) +
  geom_point(size = 4) +
  labs(
    title = "Iteration: {frame_along}",
...
p1_ani <- animate(p1, nframes = n_iter, fps = 15, width = 600, height = 400)
anim_save("images/mcmc_trace_mh.gif", animation = p1_ani)

p2_ani <- animate(p2, nframes = n_iter, fps = 15, width = 600, height = 400)
anim_save("images/mcmc_post_mh.gif", animation = p2_ani)
```

![](images/mcmc_trace_mh.gif)

![](images/mcmc_post_mh.gif)
...
```{r}
.libPaths(c("/home/rstudio/vscode-R/renv/library/linux-ubuntu-noble/R-4.5/x86_64-pc-linux-gnu", .libPaths()))

#–– initialize empty tibbles with the right columns ––
...
for (tau in seq_len(T)) {
  # 1) draw a fresh momentum
  p <- rnorm(1, 0, scale_p)

  # 2) record that "kick" (you had this already)
  step <- bind_rows(
    step,
    tibble(temp = 2, p = p, prev_p = prev_p, hamiltonian = 0, gr = tau)
  )

  # 3) run the leapfrog chain for L steps
  one_step <- move_one_step(theta, shape, rate,
                            p, eps = eps, L = L,
                            stlide = 12) %>%
    as_tibble() %>%
    mutate(gr = tau)  # group id for the whole trajectory

  # 5) append
  step  <- bind_rows(step, one_step)
  # 6) update for the next iteration
  theta  <- one_step$theta[nrow(one_step)]
  prev_p <- one_step$p[nrow(one_step)]
}


# 1) Tidy the point data --------------------------------------------------
step_anim <- step %>%
  filter(temp != 2) %>%  # drop your “kick” markers
  mutate(
    gr    = factor(gr),            # grouping by Hamiltonian chunk
    time  = row_number()           # frame index
  )
...
step_segs <- step_anim %>%
  transmute(
    ...
    theta_end   = lead(theta),
    p_end       = lead(p),
    # only draw at the *current* time
    time       = time,
    # flag if this is the boundary between two Hamiltonians
    is_jump     = (gr != lead(gr))
  ) #%>%
  # filter(is_jump)  # keep only those that cross gr → lead(gr)


p4 <- ggplot() +
  # static Hamiltonian contours
  geom_contour(
    data = cont_dat,
    aes(x = theta, y = p, z = hamiltonian),
    bins = 50, size = 0.2
  ) +
  # dashed path of all past leapfrog points
  geom_path(
    data = step_anim,
    aes(x = theta, y = p, group = gr),
    linetype = "dashed"
  ) +
  # cumulative blue points
  geom_point(
    data = step_anim,
    aes(x = theta, y = p),
    colour = "blue", size = 2
  ...
  coord_cartesian(xlim = c(-0.2, 3.5),
                  ylim = c(-5,     5   )) +
  theme_light() +
  labs(title = "Leapfrog HMC — step {current_frame}")  +
...
p4_ani <- animate(
  p4,
  nframes  = max(step_anim$time),  # = nrow(step)
  fps      = 20,
  width    = 480,
  height   = 240,
  renderer = gifski_renderer("images/leapfrog.gif")
)
...
| Typical software           | JAGS, custom R                     | JAGS, BUGS                                | Stan, brms, cmdstanr                      |
